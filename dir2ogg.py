...
# Copyright (C) 2015      Simon Kovacs <s.k@riseup.net>
#
#
...
def read_opts():
    ...
    parser.add_option('-q', '--quality', metavar='N', default=5.0, type='float', help='quality. N is a number from 1-10 (default %default)')
    ...
    parser.add_option('-a', '--convert-all', action='store_true', help="convert all supported formats (not needed anymore)", default = True)
    ...
    parser.add_option('--delete-input', action='store_true', help='delete input files. Be careful when using this option!')
    ...
    parser.add_option('-o','--output-dir', action='store', type='string', dest='destination', help='Specify the output directory.', default = '~/vorbis/')
    parser.add_option('-O', '--overwrite', action='store_true', help='Overwrite already existing .ogg files. Skip otherwise.', default = False)
    parser.add_option('-N','--naming-scheme', action='store', type='string', dest='naming_scheme', help='Specify the naming Scheme. Keywords are %artist %album %tracknumber %year %disc %title %album', default = '%artist/%album/%tracknumber - %title')
...
class Convert(Id3TagHandler):
    ...
    def __init__(self, song, conf):
        ...
        song_artist=self.filtertag(self.tags.get('artist')[0])
        song_title=self.filtertag(self.tags.get('title')[0])
        song_album=self.filtertag(self.tags.get('album')[0])
        song_tracknumber=self.filtertag(self.tags.get('tracknumber')[0])
        song_date = self.filtertag(self.tags.get('date')[0])
        song_disc = self.filtertag(self.tags.get('discnumber')[0])
        song_dir = self.conf.destination



        scheme = self.conf.naming_scheme
        titles = re.finditer(r"(\%title)+",scheme)
        tracknumbers = re.finditer(r"(\%tracknumber)+",scheme)
        years = re.finditer(r"(\%year)+",scheme)
        albums = re.finditer(r"(\%album)+",scheme)
        artists = re.finditer(r"(\%artist)+",scheme)
        disks = re.finditer(r"(\%disc)+",scheme)

        all_signifiers = []

        for m in artists:
            all_signifiers.append(Signifier(m,song_artist))
            print("appended")

        for m in titles:
            all_signifiers.append(Signifier(m,song_title))

        for m in tracknumbers:
            all_signifiers.append(Signifier(m,song_tracknumber))


        for m in years:
            all_signifiers.append(Signifier(m,song_date))


        for m in albums:
            all_signifiers.append(Signifier(m,song_album))

        for m in disks:
            all_signifiers.append(Signifier(m,song_disc))


        all_signifiers.sort(key=Signifier.get_key,reverse=True)
        filename = scheme
        for sig in all_signifiers:
            leftindex = sig.get_start()
            rightindex = sig.get_end()
            left = filename[0:leftindex]
            right = filename[rightindex:]
            filename = left + sig.get_replacement() + right
        ...
        song_root = self.conf.destination + filename
        self.songwav = song_root + '.wav'
        self.songogg = song_root + '.ogg'
        if self.conf.overwrite == False:
            if os.path.exists(self.songogg):
                print ("File "+self.songogg + " already exists. Skipping")
                return
        self.convert()
    ...
    def convert(self):
        ...
        if dec:
            ...
            enc = call(['oggenc', '-o', self.songogg, '-q', str(self.conf.quality).replace('.', ','), self.songwav] + opts)
            if enc != 0:
...
def show_license(*args, **kwargs):
    ...
    print 'Copyright (C) 2015      Simon Kovacs <s.k@riseup.net>\n'
    print 'This program is distributed in the hope that it will be useful,'
    print 'but WITHOUT ANY WARRANTY; without even the implied warranty of'
    print 'MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the'
    print 'GNU General Public License for more details.\n'
    print 'Currently developed by Simon Kovacs <s.k@riseup.net>.'
    sys.exit(0)

def main():
    conf = read_opts()
    conf_args, conf = conf[1], conf[0]
    destin = os.path.expanduser(conf.destination)

    if destin[-1] != r"/":
        destin = destin + r"/"
        if os.path.isdir(destin) != True:
            try:
                os.makedirs(destin)
            except OSError as exception:
                print ("Location " + destin + " either not writable or malformed.")
                return
    conf.destination = destin
    if conf.cdda:
